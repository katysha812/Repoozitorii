...
from PyQt6.QtWidgets import (
    ...
    QStackedWidget, QSplitter, QTabWidget, QFileDialog
)
...
class GraphsDialog(QDialog):
    ...
    def create_custom_tab(self):
        ...
        # Тип графика (только гистограмма)
        self.custom_graph_type = QComboBox()
        self.custom_graph_type.addItem("Гистограмма") #  Пока только гистограмма
    ...
    def plot_occupancy_graph(self):
        ...
        if graph_type == "Гистограмма":
            ax.hist(room_ids, bins=len(set(room_ids)), rwidth=0.8) # Уменьшил ширину столбцов
...
class HotelApp(QMainWindow):
    def __init__(self):
        ...
        self.btn_graphs = QPushButton("Графики") # Добавлена кнопка "Графики"
        ...
        button_layout.addWidget(self.btn_graphs) # Добавлена кнопка "Графики"
        ...
        self.btn_graphs.clicked.connect(self.open_graphs_dialog) # Connect the new button

        self.stacked_widget.setCurrentWidget(self.clients_widget)
    ...
    def edit_client(self, index):
        client_id = int(self.clients_model.item(index.row(), 0).text())
        client = self.session.query(Проживающий).get(client_id)
        ...
        self.passport_issue_date = QDateEdit()
        if passport:
            self.passport_issue_date.setDate(QDate.fromString(passport.дата_выдачи.strftime("%Y-%m-%d"), "yyyy-MM-dd"))
        else:
            self.passport_issue_date.setDate(QDate.currentDate())
    ...
    def save_client(self, dialog, client=None):
        try:
            ...
            if (self.passport_series.text() and self.passport_number.text() and 
                self.passport_issued_by.text()):
                
                passport_data = {
                    "серия": self.passport_series.text(),
                    "номер": self.passport_number.text(),
                    "выдан": self.passport_issued_by.text(),
                    "дата_выдачи": self.passport_issue_date.date().toPyDate(),
                    "место_рожд": self.passport_birth_place.text() or None
                }
                
                if client.паспорт:
                    for key, value in passport_data.items():
                        setattr(client.паспорт, key, value)
                else:
                    passport = Паспорт(**passport_data)
    ...
    def edit_service(self, index):
        service_id = int(self.services_model.item(index.row(), 0).text())
        service = self.session.query(Услуга).get(service_id)
        ...
        self.service_price = QLineEdit(str(service.цена))
    ...
    def save_service(self, dialog, service=None):
        try:
            if not self.service_name.text():
                raise ValueError("Название услуги обязательно")
            
            price = float(self.service_price.text())
            if price <= 0:
                raise ValueError("Цена должна быть положительным числом")
            
            if service is None:
                service = Услуга(
                    название=self.service_name.text(),
                    описание=self.service_description.text() or None,
                    цена=price
                )
                self.session.add(service)
            else:
                service.название = self.service_name.text()
                service.описание = self.service_description.text() or None
                service.цена = price
    ...
    def edit_booking(self, index):
        ...
        for i, c in enumerate(clients):
            self.booking_client.addItem(f"{c.фамилия} {c.имя}", c.id)
            if c.id == booking.id_проживающего:
                current_client_idx = i
        self.booking_client.setCurrentIndex(current_client_idx)
        layout.addRow("Клиент:", self.booking_client)
        ...
        for i, r in enumerate(rooms):
            self.booking_room.addItem(f"№{r.id} ({r.тип_номера.название}, {r.стоимость_сутки} руб.)", r.id)
            if r.id == booking.id_номера:
                current_room_idx = i
        self.booking_room.setCurrentIndex(current_room_idx)
        layout.addRow("Номер:", self.booking_room)
    ...
    def delete_booking(self, dialog, booking):
        reply = QMessageBox.question(
            self, "Подтверждение", 
