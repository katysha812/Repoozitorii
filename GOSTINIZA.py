...
class HotelApp(QMainWindow):
    ...
    def delete_client(self, dialog, client):
        # Проверяем наличие связанных бронирований
        bookings = self.session.query(Бронь).filter_by(id_проживающего=client.id).all()
        
        if bookings:
            reply = QMessageBox.question(
                self, "Подтверждение",
                f"У клиента {client.фамилия} {client.имя} есть активные бронирования. "
                "Удалить клиента и все его бронирования?",
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                # Удаляем все связанные бронирования и их услуги
                for booking in bookings:
                    # Удаляем услуги бронирования
    ...
    def edit_booking(self, index):
        client_item = self.bookings_model.item(index.row(), 0)
        if not client_item:
            return
        
        client_name = client_item.text()
        surname, name = client_name.split(" ", 1)[0], client_name.split(" ", 1)[1] if " " in client_name else ""
        
        # Находим бронирование по клиенту и датам
        booking = self.session.query(Бронь).join(Проживающий).filter(
            Проживающий.фамилия == surname,
            Проживающий.имя == name
        ).first()
    ...
    def delete_booking(self, dialog, booking):
        reply = QMessageBox.question(
            self, "Подтверждение",
            f"Удалить бронирование клиента {booking.проживающий.фамилия} {booking.проживающий.имя}?",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
        )
        if reply == QMessageBox.StandardButton.Yes:
            try:
