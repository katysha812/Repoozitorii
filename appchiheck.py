...
class LoginWindow(QWidget):
    def __init__(self):
        super().__init__()
        app.setWindowIcon(QtGui.QIcon('iconMain.png'))
    ...
    def save_progress(self):
        ...
        with Session() as session:
            if self.progress:
                ...
                progress = session.get(Progress, progress.id)
                progress.date = date
...
class AdminWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        app.setWindowIcon(QtGui.QIcon('iconAdmin.png'))
        self.setWindowTitle("Фитнес-трекер - Администратор")
        self.setGeometry(100, 100, 800, 600)

        # Создаем основной layout
        self.main_layout = QVBoxLayout() #Создаем здесь
        ...
        self.main_layout.addWidget(self.tab_widget) #Добавляем tab виджет в основной layout
    ...
    def filter_users(self):
        selected_goal_id = self.goal_filter.currentData()

        with Session(engine) as session:
            # Базовый запрос
            query = session.query(User)

            # Добавляем фильтр, если выбрана цель
            if selected_goal_id:
                query = query.filter(User.goal_id == selected_goal_id)
    ...
    def search_users(self):
        ...
        with Session(engine) as session:
            ...
            # Добавляем фильтр, если есть текст для поиска
            if search_text:
                query = query.filter(User.name.like(f"%{search_text}%"))  # Поиск по имени
    ...
    def create_preferences_tab(self):
        ...
        self.preferences_table.setColumnCount(1)
        self.preferences_table.setHorizontalHeaderLabels(["Название"])
        self.preferences_table.setColumnWidth(0, 200)
    ...
    def create_goals_tab(self):
        ...
        self.goals_table.setColumnCount(1)
        self.goals_table.setHorizontalHeaderLabels(["Название"])
        self.preferences_table.setColumnWidth(0, 200)
    ...
    def create_users_tab(self):
        ...
        self.users_table.setHorizontalHeaderLabels(["Имя", "Email", "Пол", "Рост", "Вес (цель)", "Цель","Предпочтение"])
        self.users_table.setColumnWidth(0, 200)
        self.users_table.setColumnWidth(1, 200)

        # Создаем поле для поиска пользователей
        self.search_input = QLineEdit()
        self.search_button = QPushButton("Поиск")
        self.search_button.clicked.connect(self.search_users)

        # Создаем выпадающий список для фильтрации по цели
        self.goal_filter = QComboBox()
        self.goal_filter.addItem("Все цели", None)  # Добавляем пункт "Все цели"
        with Session() as session:
            goals = session.query(Goal).all()
            for goal in goals:
                self.goal_filter.addItem(goal.name, goal.id)
        self.goal_filter.currentIndexChanged.connect(self.filter_users)
        ...
        # Создаем layout для вкладки "Пользователи"
        layout = QVBoxLayout()
        layout.addWidget(QLabel("Поиск пользователя:"))
        layout.addWidget(self.search_input)
        layout.addWidget(self.search_button)
        layout.addWidget(QLabel("Фильтр по цели:"))
        layout.addWidget(self.goal_filter)
    ...
    def create_workouts_tab(self):
        ...
        self.workouts_table.setHorizontalHeaderLabels(
            ["Пользователь", "Дата", "Тип тренировки", "Длительность", "Калории"]
        )
        # Исправленная строка:
        self.workouts_table.setEditTriggers(QTableWidget.EditTrigger.NoEditTriggers)
    ...
    def load_users(self, users=None):
        with Session() as session:
            ...
            for i, user in enumerate(users):
                ...
                # Сохраняем ID пользователя в скрытых данных первого столбца
    ...
    def edit_preference(self):
        ...
        with Session() as session:
            preference = session.get(Preference, pref_id)
    ...
    def edit_user(self):
        ...
        # Получаем ID из скрытых данных первого столбца
    ...
    def edit_exercise(self):
        ...
        with Session() as session:
            exercise = session.get(ExerciseName, exercise_id)
    ...
    def search_workouts(self):
                ...
                with Session() as session:
                    ...
                    for workout in workouts:
                        # Получаем worktype для каждой тренировки
                        worktype = session.get(WorkType, workout.worktype_id)

                        # Проверяем, содержит ли название типа тренировки текст поиска
                        if worktype and search_text in worktype.name.lower():
                            ...
                            self.workouts_table.setItem(row, 1, QTableWidgetItem(worktype.name))
                            self.workouts_table.setItem(row, 2, QTableWidgetItem(str(workout.time)))
                            self.workouts_table.item(row, 0).setData(Qt.ItemDataRole.UserRole, workout.id)
    ...
    def save_preference(self):
        ...
        with Session() as session:
            if self.preference:
                ...
                preference = session.get(Preference, self.preference.id)
    ...
    def save_goal(self):
        ...
        with Session() as session:
            if self.goal:
                ...
                goal = session.get(Goal, self.goal.id)
                goal.name = name
            else:
                # Создаем новую цель
                goal = Goal(
...
class ExerciseWindowAdmin(QDialog):
    ...
    def save_exercise(self):
        ...
        with Session() as session:
            if self.exercise:
                # Редактируем существующее упражнение
                exercise = session.get(ExerciseName, self.exercise.id)
                exercise.name = name
